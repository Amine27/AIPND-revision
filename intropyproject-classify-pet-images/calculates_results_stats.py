...
def calculates_results_stats(results_dic):
    ...
    n_correct_dogs, n_dogs_img, n_correct_notdogs, n_notdogs_img, n_correct_breed, n_match = 0, 0, 0, 0, 0, 0

    for image, label in results_dic.items():
        if(label[3] == 1 and label[4] == 1):
            n_correct_dogs += 1
        if(label[3] == 1):
            n_dogs_img += 1
    ...
    if(n_notdogs_img > 0):
        pct_correct_non_dogs = (n_correct_notdogs / n_notdogs_img) * 100
    if(n_images > 0):
        pct_correct_match = (n_match / n_images) * 100
    ...
    results_stats_dic = {'n_images': n_images,
                         ...
                         'pct_correct_match': pct_correct_match}
